...
const Template04 = ({ data, disabled=false, onClick=()=>{}, user=null }) => {
  ...
  const thisData = useMemo(() => {
    return {
      ...
      Field3: replaceRules(data?.Field3 || { type: 'text', value: '[FirstNameEN] [LastNameEN]', color: '#ffffff' }, user),
      Field4: replaceRules(data?.Field4 || { type: 'text', value: '[PositionEN]', color: '#ffffff' }, user),
      Field5: replaceRules(data?.Field5 || { type: 'text', value: '[DepartmentEN]', color: '#ffffff' }, user),
